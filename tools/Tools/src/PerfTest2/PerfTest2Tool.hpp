        ...
        try {
            ...
            sub->addSymbols(parsedSymbols | ranges::views::transform([](auto &&s) {
                                return TimeSeriesSubscriptionSymbol(
                                    CandleSymbol::valueOf(s.asStringSymbol(), {CandlePeriod::DAY}), fromTime);
                            }));
            sub2->addSymbols(parsedSymbols);
            sub3->addSymbols(
                parsedSymbols | ranges::views::transform([](auto &&s) {
                    return TimeSeriesSubscriptionSymbol(
                        CandleSymbol::valueOf(s.asStringSymbol(), {CandlePeriod::valueOf(1, CandleType::MINUTE)}), fromTime);
                }));
