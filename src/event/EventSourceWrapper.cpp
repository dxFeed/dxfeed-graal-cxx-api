...
std::ptrdiff_t EventSourceWrapper::EventSourceListUtils::calculateGraalListSize(std::ptrdiff_t initSize) noexcept {
    return GraalListUtils<dxfg_indexed_event_source_list, EventSourceWrapper>::calculateSize(initSize);
}

void *EventSourceWrapper::EventSourceListUtils::newGraalList(std::ptrdiff_t size) {
    return GraalListUtils<dxfg_indexed_event_source_list, EventSourceWrapper>::newList(size);
}

bool EventSourceWrapper::EventSourceListUtils::setGraalListElement(void *graalList, std::ptrdiff_t elementIdx,
                                                                   void *element) noexcept {
    return GraalListUtils<dxfg_indexed_event_source_list, EventSourceWrapper>::setElement(graalList, elementIdx,
                                                                                          element);
}

bool EventSourceWrapper::EventSourceListUtils::freeGraalListElements(void *graalList, std::ptrdiff_t count) {
    return GraalListUtils<dxfg_indexed_event_source_list, EventSourceWrapper>::freeElements(graalList, count);
}

void EventSourceWrapper::EventSourceListUtils::freeGraalList(void *graalList) {
    GraalListUtils<dxfg_indexed_event_source_list, EventSourceWrapper>::freeList(graalList);
}

std::vector<EventSourceWrapper> EventSourceWrapper::EventSourceListUtils::fromGraalList(void *graalList) {
    return GraalListUtils<dxfg_indexed_event_source_list, EventSourceWrapper>::fromList(graalList);
}

template void *EventSourceWrapper::EventSourceListUtils::toGraalList<dxfcpp::EventSourceWrapper const *>(
