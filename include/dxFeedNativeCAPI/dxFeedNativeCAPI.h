...
/**
 * Represents the role of endpoint that was specified during its @ref dxfc_endpoint_create() "creation".
 */
typedef enum dxfc_endpoint_role_t {
     ...
     * delayed data processing (<b>this is a default role</b>). dxfc_endpoint_get_feed() function
    ...
    /**
     * `ON_DEMAND_FEED` endpoint is similar to ::DXFC_ENDPOINT_ROLE_FEED, but it is designed to be used with dxfc_on_demand_service_t for
     * historical data replay only.
     *
     * `ON_DEMAND_FEED` endpoint cannot be connected to an ordinary data feed at all.
     */
    DXFC_ENDPOINT_ROLE_ON_DEMAND_FEED = 1,

    /**
     * `STREAM_FEED` endpoint is similar to ::DXFC_ENDPOINT_ROLE_FEED and also connects to the remote data feed provider,
     * but is designed for bulk parsing of data from files. dxfc_endpoint_get_feed() function
     ...
     * Note, that in this role, dxfc_feed_get_last_event() function does not work.
     */
    DXFC_ENDPOINT_ROLE_STREAM_FEED = 2,

    /**
     * `PUBLISHER` endpoint connects to the remote publisher hub (also known as multiplexor) or
     * creates a publisher on the local host. dxfc_endpoint_get_publisher() function returns
     * a publisher object that publishes events to all connected feeds.
     * Note, that in this role, dxfc_feed_get_last_event() function does not work and
     * time-series subscription is not supported.
     *
     * This endpoint is automatically connected to the configured data feed.
     */
    DXFC_ENDPOINT_ROLE_PUBLISHER = 3,

    /**
     * `STREAM_PUBLISHER` endpoint is similar to ::DXFC_ENDPOINT_ROLE_PUBLISHER and also connects to the remote publisher
     * hub, but is designed for bulk publishing of data. dxfc_endpoint_get_publisher() function returns a
     * publisher object that publishes events to all connected feeds. Published events are not conflated, are not
     * skipped, and are processed as fast as possible. Note, that in this role, dxfc_feed_get_last_event() function
     * does not work and time-series subscription is not supported.
     */
    DXFC_ENDPOINT_ROLE_STREAM_PUBLISHER = 4,

    /**
     * `LOCAL_HUB` endpoint is a local hub without ability to establish network connections.
     * Events that are published via @ref dxfc_endpoint_get_publisher() "publisher" are delivered to local
     * @ref dxfc_endpoint_get_feed() "feed" only.
     */
    DXFC_ENDPOINT_ROLE_LOCAL_HUB = 5,
...
typedef enum dxfc_endpoint_state_t {
    ...
    /**
     * The @ref dxfc_endpoint_connect() "connect" function was called to establish connection to
     * remove endpoint, but connection is not actually established yet or was lost.
     */
    DXFC_ENDPOINT_STATE_CONNECTING = 1,

    /**
     * The connection to remote endpoint is established.
     */
    DXFC_ENDPOINT_STATE_CONNECTED = 2,

    /**
     * Endpoint was @ref dxfc_endpoint_close() "closed".
     */
    DXFC_ENDPOINT_STATE_CLOSED = 3,
} dxfc_endpoint_state_t;

/**
 * The endpoint current state change listener
 *
 * @see dxfc_endpoint_add_state_change_listener()
 * @see dxfc_endpoint_remove_state_change_listener()
 */
 ...
 * This can be forced by calling the dxfc_endpoint_free() function.
 */
typedef void *dxfc_endpoint_t;

/**
 * The dxFeed endpoint's builder handle. It is used for searching, adding builders to the "registry", deleting builders from
 * the "registry" and calling functions associated with a particular builder.
 *
 * The "registry" is responsible for neatly closing resources, shutting down work, and freeing up memory on shutdown.
 * This can be forced by calling the dxfc_endpoint_builder_free() function.
 */
 ...
 * Use dxfc_endpoint_builder_build to build an instance of dxFeed endpoint when all configuration properties were set.
 *
 ...
 * Default role is @ref DXFC_ENDPOINT_ROLE_FEED "FEED".
 *
 ...
 * @return DXFC_EC_SUCCESS - if the operation was successful; otherwise - DXFC_EC_ERROR.
 */
dxfc_error_code_t dxfc_endpoint_builder_supports_property(dxfc_endpoint_builder_t builder, const char *key,
 ...
 * @return DXFC_EC_SUCCESS - if the operation was successful; otherwise - DXFC_EC_ERROR.
 */
dxfc_error_code_t dxfc_endpoint_builder_build(dxfc_endpoint_builder_t builder, void *user_data,
...
/**
 * Returns a default application-wide singleton instance of dxFeed endpoint with a @ref DXFC_ENDPOINT_ROLE_FEED "FEED" role.
 * Most applications use only a single data-source and should rely on this function to get one.
 * This function creates an endpoint on the first use with a default
 * configuration.
 ...
 *
 *  dxfc_endpoint_get_instance2(DXFC_ENDPOINT_ROLE_FEED, user_data, &endpoint);
 *  ```
 *
 * @param user_data User data that will be passed to the endpoint's state transition listeners.
 * @param[out] endpoint The created endpoint
 ...
 * You can provide configuration via system properties.
 *
 * The configuration does not have to include an address. You can use @ref dxfc_endpoint_connect()
 * "connect(endpoint, address)" and dxfc_endpoint_disconnect() functions on the instance that is returned by this
 ...
 * application, then create a custom instance of dxFeed endpoint with dxfc_endpoint_new_builder() function, configure it,
 * and use dxfc_endpoint_builder_build() function.
 *
...
/**
 * Creates an endpoint with @ref DXFC_ENDPOINT_ROLE_FEED "FEED" role.
 * The result of this function is the same as <b>`dxfc_endpoint_create2(DXFC_ENDPOINT_ROLE_FEED, user_data, &endpoint)`</b>.
 * This is a shortcut to
 * ```c
 * dxfc_endpoint_builder_t builder;
 * dxfc_endpoint_t endpoint;
 *
 * dxfc_endpoint_new_builder(&builder);
 * dxfc_endpoint_builder_build(builder, user_data, &endpoint);
 * ```
 *
 * @param user_data User data that will be passed to the endpoint's state transition listeners.
 * @param[out] endpoint The created endpoint
 ...
 *
 * dxfc_endpoint_new_builder(&builder);
 * dxfc_endpoint_builder_with_role(builder, role);
 * dxfc_endpoint_builder_build(builder, user_data, &endpoint);
 * ```
 *
 * @param role The role of dxFeed endpoint instance
...
/**
 * Closes this endpoint. All network connection are terminated as with dxfc_endpoint_disconnect() function and no further
 * connections can be established.
 *
 * The endpoint @ref dxfc_endpoint_get_state() "state" immediately becomes @ref DXFC_ENDPOINT_STATE_CLOSED "CLOSED".
 ...
 * This function performs the same actions as dxfc_endpoint_close(), but also awaits
 * termination of all outstanding data processing tasks. It is designed to be used
 * with @ref DXFC_ENDPOINT_ROLE_STREAM_FEED "STREAM_FEED" role after dxfc_endpoint_await_not_connected() function returns
 ...
 * This function shall be called before @ref dxfc_endpoint_connect() "connect" together
 * with @ref dxfc_endpoint_password() "password" to configure service access credentials.
 *
 * @param endpoint The dxFeed endpoint
 ...
 * This function shall be called before @ref dxfc_endpoint_connect() "connect" together
 * with @ref dxfc_endpoint_user() "user" to configure service access credentials.
 *
 * @param endpoint The dxFeed endpoint
...
* This function does nothing if address does not change or if this endpoint is @ref DXFC_ENDPOINT_STATE_CLOSED "CLOSED".
* The endpoint @ref dxfc_endpoint_get_state() "state" immediately becomes @ref DXFC_ENDPOINT_STATE_CONNECTING "CONNECTING" otherwise.
*
...
* using @ref dxfc_endpoint_user() "user" and @ref dxfc_endpoint_password() "password" functions.
*
...
* notification about state transition from DXFC_ENDPOINT_STATE_NOT_CONNECTED to DXFC_ENDPOINT_STATE_CONNECTING gets processed by all
* listeners.
*
 ...
 *
 * <p>The effect of the function is alike to invoking dxfc_endpoint_disconnect() and dxfc_endpoint_connect()
 ...
 * dxfc_endpoint_connect() function or was disconnected with dxfc_endpoint_disconnect() function.
 *
 * <p>The function initiates a short-path way for reconnecting, so whether observers will have a chance to see
 * an intermediate state DXFC_ENDPOINT_STATE_NOT_CONNECTED depends on the implementation.
 *
 ...
 * This function does nothing if this endpoint is @ref DXFC_ENDPOINT_STATE_CLOSED "CLOSED".
 * The endpoint @ref dxfc_endpoint_get_state() "state" immediately becomes @ref DXFC_ENDPOINT_STATE_NOT_CONNECTED "NOT_CONNECTED" otherwise.
 *
 * <p>This function does not release all resources that are associated with this endpoint.
 * Use dxfc_endpoint_disconnect() function to release all resources.
 *
 ...
 *
 * <p>This function does not release all resources that are associated with this endpoint.
 * Use dxfc_endpoint_close() function to release all resources.
 *
 * @param endpoint The dxFeed endpoint
 * @return DXFC_EC_SUCCESS - if the operation was successful; otherwise - DXFC_EC_ERROR.
 */
...
/**
 * Waits while this endpoint @ref dxfc_endpoint_get_state() "state" becomes @ref DXFC_ENDPOINT_STATE_NOT_CONNECTED "NOT_CONNECTED" or
 * @ref DXFC_ENDPOINT_STATE_CLOSED "CLOSED". It is a signal that any files that were opened with
 * @ref dxfc_endpoint_connect() "dxfc_endpoint_connect(endpoint, \"file:...\")" function were finished reading, but not necessary were
 * completely processed by the corresponding subscription listeners. Use dxfc_endpoint_and_await_termination() after this
 * function returns to make sure that all processing has completed.
 *
 * <p><b>This function is blocking.</b>
 *
 * @param endpoint The dxFeed endpoint
...
/**
 * Adds listener that is notified about changes in @ref dxfc_endpoint_get_state() "state" property.
 *
 * <p>Installed listener can be removed by dxfc_endpoint_remove_state_change_listener() function.
 *
 * @param endpoint The dxFeed endpoint
...
/**
 * Removes listener that is notified about changes in @ref dxfc_endpoint_get_state() "state" property.
 * It removes the listener that was previously installed with dxfc_endpoint_add_state_change_listener() function.
 *
 * @param endpoint The dxFeed endpoint
