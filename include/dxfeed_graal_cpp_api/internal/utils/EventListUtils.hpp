...
#include "dxfeed_graal_cpp_api/internal/Conf.hpp"
...
class DXFCPP_EXPORT EventListUtils final {
  ...
  public:
    template <typename EventIt> static void *toGraalList(EventIt begin, EventIt end) noexcept {
        if constexpr (Debugger::isDebug) {
            Debugger::debug("EventUtils::toGraalList(symbols = " + elementsToString(begin, end) + ")");
        }
    ...
    static std::vector<std::shared_ptr<EventType>> fromGraalList(void *graalList) noexcept;
};
